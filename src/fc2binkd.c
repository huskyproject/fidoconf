...
void printversion(FILE * fd)
{
    ...
    fprintf(fd, "%s\n\n",
            temp =
                GenVersionStr("fconf2binkd", FC_VER_MAJOR, FC_VER_MINOR, FC_VER_PATCH,
                              FC_VER_BRANCH,
                              cvs_date));
    nfree(temp);
}
    ...
    if(config)
    {
        if(ofname) /* if not defined output binkd config to stdout */
        {
            printf("Generate binkd %s file %s\n",
                   passwords_list_only ? "passwords" : "config",
                   ofname);
        }
