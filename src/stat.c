...
void upd_stat(char * file)
{
    ...
    if(OLD != NULL)
    {
        ...
        {
            msg2("Incompatible stat base", oldf);
            fclose(OLD);
            do_stat = 0;
            return;
        }
    ...
    {
        remove(newf);
        msg("New stat base is not written");
    }
...
#ifdef STAT_ALONE
            fprintf(stderr,
                    "%s, %d:%d/%d.%d: i=%d/o=%d/b=%d/d=%d ib=%d/ob=%d\n",
#else
            w_log(LL_DEBUGS, "%s, %d:%d/%d.%d: i=%d/o=%d/b=%d/d=%d ib=%d/ob=%d",
#endif
                    cur->tag, curl->link.addr.zone, curl->link.addr.net, curl->link.addr.node,
                    curl->link.addr.point, curl->link.in, curl->link.out, curl->link.bad,
                    curl->link.dupe, curl->link.inb, curl->link.outb);
            curl = curl->next;
        }
