...
char * sysAddress2str(SysAddress sysaddr)
{
    ...
    {
        sprintf(aka, "%d:%d/%d", sysaddr.main.zone, sysaddr.main.net, sysaddr.main.node);
    } /* endif */
...
void print_areas()
{
    ...
    unsigned int a[] =
    {
        AREA_BADMAILBOARD, AREA_DUPEBOARD, AREA_NETMAIL, AREA_LOCAL, AREA_ECHOMAIL
    };
    ...
    for(ii = 0; ii < 5; ii++)
    {
        ...
        for(i = 0; i < config.AreaCnt; i++)
        {
            ...
            if(area[i]->flags.storage == FE_FIDO || area[i]->flags.storage == FE_SQUISH ||
               area[i]->flags.storage == FE_JAM || area[i]->flags.storage == FE_PASSTHRU)
            {
            ...
            else
            {
                if(*area[i]->name)
                {
                    ...
                    {
                        fprintf(f_hpt,
                                "# !!! # area \"%s\" stored in HUDSON messagebase, doesn't support by Husky\n",
                                area[i]->name);
                    }
                }
            } /* endif */
...
void print_links()
{
    ...
    for(i = 0; i < config.NodeCnt; i++)
    {
        ...
        if(node[i]->addr.zone != node[i]->arcdest.zone ||
           node[i]->addr.net != node[i]->arcdest.net ||
           node[i]->addr.node != node[i]->arcdest.node ||
           node[i]->addr.point != node[i]->arcdest.point)
        {
 ...
 */
        ...
        {
            fprintf(f_hpt,
                    "Export                   off   # This is not precision usage \"Echomail flavour: No attach\" from fastecho's configuration");
        }
...
int parseFEconfig()
{
    ...
    while(c < config.offset)
    {
        ...
        {
            fprintf(stderr,
                    "%s file seems to be currupt (exp %ld, found %ld)\n",
                    FEconfig,
                    (long)c + FE_CONFIG_SIZE,
                    (long)ftell(f_cfg));
            fclose(f_cfg);
            return 4;
        }
...
int main(int argc, char ** argv)
{
    ...
    *pp   = '\0'; /* Trim after number */
    ...
    fprintf(f_hpt,
            "# fastecho v1.46 config (fastecho.cfg) -> %s. (c) 2:5020/960@FidoNet\n",
            output_file);
 ...
 */
    fprintf(f_hpt, "\n");
    fprintf(f_hpt,
            "AreafixKillRequests      %s\n",
            config.AreaFixFlags & KEEPREQUEST ? "off" : "on");
    fprintf(f_hpt,
            "areafixQueryReports      %s\n",
