...
int writeEchoArea(FILE * f, s_area * area, char type)
{
    ...
    if(export_scanprivmail)
    {
        fprintf(f, "Scan_PrivMail Yes\n");
    }
...
int writeFileArea(FILE * f, s_area * area)
{
    ...
    if(export_scannewfiles)
    {
        fprintf(f, "Scan_NewFiles Yes\n");
    }
 ...
 */
    ...
    else
    {
        ...
        else
        {
            ...
            {
                export_scanprivmail = export_scannewfiles = 1;
            }
            else
            {
                if(stricmp(line, "-cd") == 0)
                {
                    bbsdeffmt = 0;
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            if(stricmp(line, "-nt") == 0)
                            {
                                tag_type = TAG_NAME;
                            }
                            else
                            {
                                if(stricmp(line, "-nd") == 0)
                                {
                                    tag_type = TAG_DESC;
                                }
                                else
                                {
                                    if(stricmp(line, "-nb") == 0)
                                    {
                                        tag_type = TAG_BOTH;
                                    }
                                    else
...
int main(int argc, char * argv[])
{
...

    {
        ...
        printf("%s\n",
               temp =
                   GenVersionStr("fconf2tornado", FC_VER_MAJOR, FC_VER_MINOR, FC_VER_PATCH,
                                 FC_VER_BRANCH,
                                 cvs_date));
        nfree(temp);
    }
