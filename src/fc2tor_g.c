...
int isSubSetOfGroups(char * sub, char * groups, char * delms)
{
    ...
    while(*sub)
    {
        ...
        if(*sub)
        {
            ...
            while(!ok && *temp)
            {
                while(*temp && strchr(delms, *temp))
                {
                    temp++;
                }

                if(strncmp(grp, temp, len) == 0)
                {
                    ok = 1;
                }
...
{
    if(!isSubSetOfGroups(grp1, grp2, delms))
    {
        return 0;
    }

    if(!isSubSetOfGroups(grp2, grp1, delms))
    {
        return 0;
    }
