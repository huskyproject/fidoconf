...
char * getConfigFileNameForProgram(char * envVar, char * configName)
{
    ...
    if(envFidoConfig != NULL)
    {
        f = fopen(envFidoConfig, "r");
    }
...
int carbonNames2Addr(s_fidoconfig * config)
{
    ...
    for(i = 0; i < config->carbonCount; i++, cb++)
    {
        ...
        if(!found && (cb->move != 2) && !cb->extspawn)   /*  move==2 - delete */
        {
            ...
            {
                printf(
                    "Could not find area \"%s\" for carbon copy and BadArea not defined. Can't use this area for carbon copy\n",
                    cb->areaName);
                cb->area = NULL;
                rc++;
            }
...
void setConfigDefaults(s_fidoconfig * config)
{
    ...
    /* defaults for htick */
    if(config->fileDescription == NULL)
    {
        xstrcat(&config->fileDescription, "files.bbs");
    }
...
void disposeConfig(s_fidoconfig * config)
{
    ...
    {
        nfree(config->publicDir[i]);
    }
    nfree(config->publicDir);

    for(i = 0; i < config->linkCount; i++)
    {
        freeLink(config->links[i]);
    }
    nfree(config->links);
...
int grpInArray(char * group, char ** strarray, unsigned int len)
{
    ...
    for(i = 0; i < len; i++)
    {
        if(strarray[i] && strcmp(group, strarray[i]) == 0)
        {
            return 1;
        }
    }
...
void SetAppModule(e_known_moduls mod)
{
    theApp.module = mod;
}
