...
int writeArea(FILE * f, s_area * area, char type)
{
    ...
    {
        fprintf(f, " -$n\"%s\"", area->description);
    }

    if(area->group && strcmp(area->group, "0"))
    {
...
int main(int argc, char * argv[])
{
...

    {
        ...
        printf("%s\n",
               temp =
                   GenVersionStr("fconf2squish", FC_VER_MAJOR, FC_VER_MINOR, FC_VER_PATCH,
                                 FC_VER_BRANCH,
                                 cvs_date));
        nfree(temp);
    }
    ...
    if(argc < 2)
    {
        ...
        fprintf(stderr,
                "   fconf2squish - | sed \"\\/var\\/fido\\//u:\\\\\\/gi\" > /etc/fido/squish.cfg\n\n");
        return 1;
    }
    ...
    if(config != NULL)
    {
        if(argv[2] != NULL)
        {
            readDefaultConfig(argv[1], argv[2]);
        }
        else
        {
            remove(argv[1]);
