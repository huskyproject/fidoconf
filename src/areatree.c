...
int RebuildEchoAreaTree(ps_fidoconfig config)
{
    ...
    {
        if(tree_add(&echoAreaTree, fc_compareEntries, (char *)(&(config->echoAreas[i])),
                    fc_deleteEntry) == 0)
        {
    ...
    {
        if(tree_add(&echoAreaTree, fc_compareEntries, (char *)(&(config->localAreas[i])),
                    fc_deleteEntry) == 0)
        {
...
int RebuildFileAreaTree(ps_fidoconfig config)
{
    ...
    {
        if(tree_add(&fileAreaTree, fc_compareEntries, (char *)(&(config->fileAreas[i])),
                    fc_deleteEntry) == 0)
        {
